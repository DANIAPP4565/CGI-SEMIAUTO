...
def graficar_canales_zlogic(
    ...
    base_ecg = 0.95 if expandir_vertical else 0.70
    base_fono = -1.35 if expandir_vertical else -0.70
    ...
    y_fono = norm(df_fono["amplitud"].to_numpy(float), 0.55 * float(amplificar_fono)) + base_fono
    ...
    if expandir_vertical:
        ax.set_ylim(base_fono - 1.35, base_dz + 1.35)
    ax.set_yticks([])
    ax.set_xlabel("Tiempo (ms) - grillado vertical cada 20 ms")
    ax.set_title("Digitalización automática multicanal: dZ/dt + ECG verde + Fonocardiograma naranja")
...
if modo_morfo.startswith("PDF"):
    ...
    with colamp2:
        amplificar_fono = st.slider("Amplificación visual fonocardiograma naranja", 1.0, 4.0, 1.85, 0.05)
...
if "df_curva_icg" in st.session_state and "puntos_qbcx_auto" in st.session_state:
    ...
    if "canales_zlogic_pdf" in st.session_state:
        fig_canales = graficar_canales_zlogic(st.session_state["canales_zlogic_pdf"], pfinal, amplificar_fono=float(st.session_state.get("amplificar_fono_visual", 1.85)), amplificar_ecg=float(st.session_state.get("amplificar_ecg_visual", 1.15)), expandir_vertical=True)
        st.markdown("### Registro multicanal digitalizado con grilla cada 20 ms")
        st.caption("Convención visual: ECG en verde y fonocardiograma en naranja. La gráfica está expandida verticalmente para resaltar el fonocardiograma.")
