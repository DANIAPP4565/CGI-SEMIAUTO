...
def tabla_dominios_hemodinamicos(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {"Dominio": "Flujo", "Valores": f"IC {fmt(d['ci'],2)} L/min/m²; IDS {fmt(d['ids'],1)} mL/lat/m²; IRV {fmt(d['irv'],0)}", "Interpretación": f"{d['flujo_ic']}; {d['flujo_ids']}; {d['flujo_irv']}"},
        {"Dominio": "Función cardíaca", "Valores": f"FE Capan {fmt(fe_pct,1)} %", "Interpretación": d["funcion"]},
        {"Dominio": "Contractilidad", "Valores": f"IV {fmt(d['iv'],4)}; IH {fmt(d['ih'],5)}; IAC {fmt(d['iac'],4)}; ITC {fmt(d['itc'],2)}; PEP/LVET {fmt(d['rts'],3)}", "Interpretación": "Perfil contráctil morfológico derivado de la onda validada"},
        {"Dominio": "Rendimiento cardiovascular", "Valores": f"AC {fmt(d['ac'],3)}", "Interpretación": d["rendimiento"]},
        {"Dominio": "Volemia", "Valores": f"CFT {fmt(d['cft'],1)} 1/kOhm", "Interpretación": d["volemia"]},
    ])
