...
def digitalizar_curva_icg_desde_imagen(
    ...
    tiempo_inicio_ms: float = 0.0,
    tiempo_fin_ms: Optional[float] = None,
    y_borde_superior: Optional[float] = None,
    y_borde_inferior: Optional[float] = None,
) -> pd.DataFrame:
    """
    ...
    - Calibracion real del eje X usando tiempo inicial y final del eje original.
    - Calibracion opcional del eje Y usando los valores superior e inferior visibles en el grafico original.

    Requisito clinico-operativo: el recorte debe contener solo la curva dZ/dt y debe coincidir con los bordes reales del eje original.
    """
    ...
    # Escala vertical.
    # Si el operador carga los valores visibles del eje Y, se respeta la escala original:
    # y=0 px corresponde al borde superior del recorte y y=h-1 al borde inferior.
    # Si no hay calibracion vertical, se usa amplitud relativa en pixeles, conservando morfologia.
    roi_h = max(1, mask.shape[0] - 1)
    y_calibrado = (y_borde_superior is not None) and (y_borde_inferior is not None)
    if y_calibrado:
        amp = float(y_borde_superior) + (yinterp / roi_h) * (float(y_borde_inferior) - float(y_borde_superior))
        if invertir_polaridad:
            centro = (float(y_borde_superior) + float(y_borde_inferior)) / 2.0
            amp = centro - (amp - centro)
        eje_y_label = "Amplitud dZ/dt calibrada segun eje original"
        escala_y = "calibrada"
    else:
        ...
        eje_y_label = "Amplitud relativa dZ/dt"
        escala_y = "relativa_px"

    amp = suavizar_senal(amp, suavizado)
    if not y_calibrado and np.nanstd(amp) > 0:
        amp = amp - np.nanmedian(amp)

    # Escala horizontal. Se respeta el intervalo temporal visible del eje X original.
    if tiempo_fin_ms is None:
        tiempo_fin_ms = float(tiempo_inicio_ms) + float(duracion_ms)
    tiempo_inicio_ms = float(tiempo_inicio_ms)
    tiempo_fin_ms = float(tiempo_fin_ms)
    if tiempo_fin_ms <= tiempo_inicio_ms:
        raise ValueError("La calibracion del eje X no es valida: el tiempo final debe ser mayor que el inicial.")

    t = np.linspace(tiempo_inicio_ms, tiempo_fin_ms, len(amp))
    df_out = pd.DataFrame({"tiempo_ms": t, "amplitud_relativa": amp})
    df_out.attrs["eje_y_label"] = eje_y_label
    df_out.attrs["escala_y"] = escala_y
    df_out.attrs["tiempo_inicio_ms"] = tiempo_inicio_ms
    df_out.attrs["tiempo_fin_ms"] = tiempo_fin_ms
    df_out.attrs["y_borde_superior"] = y_borde_superior
    df_out.attrs["y_borde_inferior"] = y_borde_inferior
    return df_out
...
def detectar_puntos_qbcx_icg(df_curva: pd.DataFrame, fc: Optional[float] = None) -> Dict[str, Any]:
    ...
    mask_c = (trel >= c_ini_ms) & (trel <= c_fin_ms)
    if mask_c.sum() >= 8:
    ...
    b_ini_ms = float(t[ic]) - min(180.0, max(60.0, float(trel[ic]) - 15.0))
    b_ini_ms = max(float(t[0]) + 35.0, b_ini_ms)
    ...
    b_min = float(t[0]) + 35.0
    b_max = min(float(t[ic]) - 5.0, float(t[0]) + max(120.0, 0.30 * dur))
    if t[ib] < b_min:
...
def graficar_curva_icg_con_puntos(df_curva: pd.DataFrame, p: Dict[str, Any]) -> plt.Figure:
    ...
    ax.set_ylabel(df_curva.attrs.get("eje_y_label", "Amplitud relativa dZ/dt"))
...
if img_file is not None:
    ...
    st.warning(
        "Para evitar curvas deformadas o escalas incorrectas, recortar solo la señal dZ/dt promedio, "
        "haciendo coincidir el recorte con los bordes reales del grafico original. "
        "No incluir ECG, fonocardiograma, carotidograma, texto, números ni más de una curva."
    )

    fc_det = obtener_fc_desde_df(df_final)
    ciclo_sug = ciclo_ms_desde_fc(fc_det, 800.0)
    st.markdown("### Calibracion de ejes de la curva original")
    st.caption(
        "Para que la digitalizacion tenga la misma escala que la imagen original, el recorte debe coincidir "
        "con el area real del grafico y se deben cargar los valores visibles de los ejes."
    )
    cx1, cx2, cx3 = st.columns(3)
    with cx1:
        tiempo_inicio_ms = st.number_input("Eje X: tiempo inicial visible (ms)", min_value=-1000.0, max_value=5000.0, value=0.0, step=10.0)
    with cx2:
        tiempo_fin_ms = st.number_input("Eje X: tiempo final visible (ms)", min_value=-1000.0, max_value=5000.0, value=float(round(ciclo_sug, 1)), step=10.0)
    with cx3:
        duracion_ms = max(1.0, float(tiempo_fin_ms) - float(tiempo_inicio_ms))
        st.metric("Escala X aplicada", f"{duracion_ms:.0f} ms")

    calibrar_y = st.checkbox(
        "Calibrar eje Y con los valores originales visibles",
        value=False,
        help="Activar si la imagen muestra escala vertical de dZ/dt. Si no se activa, la app conserva amplitud relativa en pixeles."
    )
    if calibrar_y:
        cy1, cy2 = st.columns(2)
        with cy1:
            y_borde_superior = st.number_input("Eje Y: valor del borde superior del grafico", value=100.0, step=1.0)
        with cy2:
            y_borde_inferior = st.number_input("Eje Y: valor del borde inferior del grafico", value=-100.0, step=1.0)
    else:
        y_borde_superior = None
        y_borde_inferior = None
    ...
    with c3:
        st.caption("Si el eje Y fue calibrado, la polaridad se invierte alrededor del centro vertical cargado.")
    ...
    if x2 > x1 and y2 > y1:
        ...
        st.caption("La escala X/Y se aplicara exactamente al ancho y alto de este recorte. Si el recorte no coincide con los bordes reales del grafico original, la escala saldra desplazada.")
    ...
    if st.button("Digitalizar curva y proponer QRS-B-C-X"):
        try:
            df_curva = digitalizar_curva_icg_desde_imagen(
                ...
                tiempo_inicio_ms=tiempo_inicio_ms,
                tiempo_fin_ms=tiempo_fin_ms,
                y_borde_superior=y_borde_superior,
                y_borde_inferior=y_borde_inferior,
            )
