...
def digitalizar_curva_icg_desde_imagen(
    ...
    arr = np.asarray(img)
    h, w = arr.shape[:2]
    crop_x1 = max(0, min(crop_x1, w - 1)); crop_x2 = max(1, min(crop_x2, w))
    crop_y1 = max(0, min(crop_y1, h - 1)); crop_y2 = max(1, min(crop_y2, h))
    roi = arr[crop_y1:crop_y2, crop_x1:crop_x2, :]
    if roi.size == 0:
        raise ValueError("La region recortada esta vacia.")
    gris = np.mean(roi, axis=2)
    mask = gris < int(umbral_oscuridad)
    xs, ys = [], []
    for x in range(mask.shape[1]):
        yidx = np.where(mask[:, x])[0]
        if len(yidx) > 0:
            xs.append(x)
            ys.append(float(np.median(yidx)))
    if len(xs) < 20:
        raise ValueError("No se detectaron suficientes puntos de curva. Ajustar recorte/umbral.")
...
def detectar_puntos_qbcx_icg(df_curva: pd.DataFrame, fc: Optional[float] = None) -> Dict[str, Any]:
    ...
    n = len(y)
    i0, i1 = max(1, int(.04*n)), min(n-2, int(.88*n))
    ic = int(i0 + np.argmax(y[i0:i1]))
    dy = np.gradient(y, t)

    # QRS/Q: inicio de ciclo, editable.
    iq = 0

    # Punto B: inflexion ascendente previa a C.
    pre_ini = max(1, idx_cercano(t, 45))
    pre_fin = max(pre_ini+3, ic-2)
    if pre_fin <= pre_ini:
        ib = max(1, ic - int(.18*n))
    else:
        ypre = y[pre_ini:pre_fin]
        dypre = dy[pre_ini:pre_fin]
        base = np.percentile(ypre, 20)
        pico = y[ic]
        amp = max(1e-6, pico-base)
        candidatos = np.where(ypre > base + .12*amp)[0]
        ib_umbral = pre_ini + int(candidatos[0]) if len(candidatos) else pre_ini + int(np.argmax(dypre))
        ib_pend = pre_ini + int(np.argmax(dypre))
        ib = int(round((ib_umbral + ib_pend)/2))
        ib = max(1, min(ib, ic-1))

    # Restringir B segun regla operativa: no antes de 50 ms y no despues de 30% del ciclo.
    dur = float(t[-1] - t[0])
    b_min = 50.0
    b_max = 0.30 * dur
    if t[ib] < b_min:
    ...
    # Punto X: nadir posterior a C, no valle Y tardio.
    post_ini = min(n-2, ic + max(2, int(.04*n)))
    post_fin = min(n-1, ic + max(8, int(.45*n)))
    if post_fin <= post_ini:
        ix = min(n-1, ic + int(.25*n))
    else:
        ix = post_ini + int(np.argmin(y[post_ini:post_fin]))

    return armar_resultado_puntos(df_curva, iq, ib, ic, ix)
...
def generar_pdf_informe(df: pd.DataFrame, morfo: Optional[Dict[str, Any]] = None) -> bytes:
    ...
    if morfo is not None and morfo.get("df_curva") is not None and morfo.get("puntos") is not None:
        ...
        elems.append(tt)
        elems.append(Spacer(1, 8))
        elems.append(Paragraph("Nota: el modulo es semiautomatico; la aceptacion final de cursores corresponde al operador medico.", styles["NormalSmall"]))
...
if not df_final.empty:
    ...
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.subheader("4. Base de ecuaciones y variables recalculadas ICG")
    basal_tmp, _ = seleccionar_basal_y_parado(df_final)
    morfo_tmp = None
    if "puntos_qbcx_final" in st.session_state:
        morfo_tmp = {"puntos": st.session_state["puntos_qbcx_final"]}
    df_derivadas = calcular_variables_icg_derivadas(basal_tmp or {}, morfo_tmp)
    st.markdown("**Variables recalculadas disponibles con los datos actuales**")
    ...
    with st.expander("Ver base de conocimiento de ecuaciones ICG"):
        ...
        st.caption("Ea/Ees se recalcula con PAS, PAD, SV/DS y tiempos PEP/LVET. Si se corrigen cursores en la curva digitalizada, esos tiempos tienen prioridad para el cálculo de FE Capan, Ees y acoplamiento ventriculo-arterial. Las ecuaciones de Kubicek y Sramek-Bernstein requieren señal dZ/dt calibrada en amplitud absoluta.")
    st.markdown("</div>", unsafe_allow_html=True)
else:
...
if img_file is not None:
    ...
    with c3:
        invertir = st.checkbox("Invertir polaridad", value=False)
    ...
    if st.button("Digitalizar curva y proponer QRS-B-C-X"):
        try:
            df_curva = digitalizar_curva_icg_desde_imagen(imagen, x1, x2, y1, y2, duracion_ms, umbral, invertir, suav)
            ...
            st.success("Curva digitalizada y cursores propuestos. Validar/corregir manualmente debajo.")
        except Exception as e:
...
if "df_curva_icg" in st.session_state and "puntos_qbcx_auto" in st.session_state:
    ...
    if not df_final.empty:
        ...
        df_der_morfo = calcular_variables_icg_derivadas(basal_para_morfo or {}, {"puntos": pfinal})
        st.markdown("### Variables recalculadas integrando los cursores morfologicos")
