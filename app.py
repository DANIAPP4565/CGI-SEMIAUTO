...
def _digitalizar_banda_trazo(
    ...
    # Si no hay trazo azul suficiente, recién ahí usar oscuro no textual como último recurso.
    if mask.sum() < 20:
        mask_oscura = (gris < umbral_oscuridad) & (sat > 12)
        if excluir_relleno_amarillo:
    ...
    if mask.size == 0 or mask.sum() < 10:
        raise ValueError(f"No se detectó trazo azul suficiente en canal {nombre_canal}.")
...
def digitalizar_pdf_zlogic_sector_superior_derecho(
    ...
    df_fono = _digitalizar_banda_trazo(
        arr, 0.790, 0.900, rr, umbral_oscuridad=150, suavizado=1,
