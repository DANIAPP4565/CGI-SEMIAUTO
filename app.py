...
# - Digitaliza automaticamente el panel superior derecho del PDF Z-Logic: dZ/dt + ECG + fonocardiograma.
...
def _digitalizar_banda_trazo(
    ...
    """Digitaliza una banda horizontal dentro del panel superior derecho: dz/dt, ECG o Fono."""
    h, w = roi_rgb.shape[:2]
    y1 = int(max(0, min(h-1, y_ini_frac*h)))
    y2 = int(max(y1+3, min(h, y_fin_frac*h)))
    banda = roi_rgb[y1:y2, :, :]
    bf = banda.astype(float)
    gris = np.mean(bf, axis=2)
    maxc = np.max(bf, axis=2)
    minc = np.min(bf, axis=2)
    sat = maxc - minc

    # Trazos coloreados primero: evita confundir texto negro con curva azul.
    mask_color = (sat > 35) & (gris < 230)
    mask_oscura = (gris < umbral_oscuridad)
    mask = mask_color if mask_color.sum() >= 20 else (mask_oscura | mask_color)
    mask = _limpiar_mascara_grafica(mask)
    if mask.sum() < 20:
        mask = _limpiar_mascara_grafica(mask_oscura)
    if mask.sum() < 20:
        mask = (gris < min(190, umbral_oscuridad + 35))
        mask = _limpiar_mascara_grafica(mask)
    ...
    amp_px = suavizar_senal(amp_px, suavizado)
...
def digitalizar_pdf_zlogic_sector_superior_derecho(
    ...
    # Bandas observadas en el panel: superior dz/dt, media ECG, inferior fono.
    df_dz = _digitalizar_banda_trazo(arr, 0.02, 0.43, rr, umbral_oscuridad=155, suavizado=suavizado, invertir=invertir_dzdt, max_salto_px=max(6, int(h*0.06)), amplitud_mm_por_px=mm_por_px, escala_y_por_mm=escala_dzdt)
    df_ecg = _digitalizar_banda_trazo(arr, 0.37, 0.68, rr, umbral_oscuridad=155, suavizado=max(3, suavizado), invertir=False, max_salto_px=max(6, int(h*0.07)))
    df_fono = _digitalizar_banda_trazo(arr, 0.62, 0.96, rr, umbral_oscuridad=155, suavizado=max(3, suavizado), invertir=False, max_salto_px=max(6, int(h*0.07)))
...
def graficar_canales_zlogic(canales: Dict[str, Any], puntos: Optional[Dict[str, Any]] = None) -> plt.Figure:
    """Grafica dZ/dt, ECG y Fono con grillado vertical cada 20 ms."""
    ...
    y_dz = norm(df_dz["amplitud"].to_numpy(float), 0.80) + 2.2
    y_ecg = norm(df_ecg["amplitud"].to_numpy(float), 0.45) + 0.7
    y_fono = norm(df_fono["amplitud"].to_numpy(float), 0.35) - 0.7

    fig, ax = plt.subplots(figsize=(12.5, 5.2))
    ax.plot(t, y_dz, linewidth=2.0, label="dZ/dt digitalizada")
    ax.plot(df_ecg["tiempo_ms"], y_ecg, linewidth=1.4, label="ECG digitalizado")
    ax.plot(df_fono["tiempo_ms"], y_fono, linewidth=1.2, label="Fonocardiograma digitalizado")

    # Grillado cada 20 ms, como solicitó el usuario.
    ticks20 = np.arange(0, rr + 20, 20)
    for x in ticks20:
        ax.axvline(x, color="#D6DEE8", linewidth=0.55, alpha=0.65, zorder=0)
    for x in np.arange(0, rr + 100, 100):
        ax.axvline(x, color="#9FB3C8", linewidth=0.9, alpha=0.85, zorder=0)

    ax.axhline(2.2, color="#B0B0B0", linewidth=0.7, alpha=0.5)
    ax.axhline(0.7, color="#B0B0B0", linewidth=0.7, alpha=0.5)
    ax.axhline(-0.7, color="#B0B0B0", linewidth=0.7, alpha=0.5)
    ax.text(-0.02*rr, 2.2, "dZ/dt", ha="right", va="center", fontsize=10, fontweight="bold")
    ax.text(-0.02*rr, 0.7, "ECG", ha="right", va="center", fontsize=10, fontweight="bold")
    ax.text(-0.02*rr, -0.7, "Fono", ha="right", va="center", fontsize=10, fontweight="bold")

    if puntos is not None:
        # Dibujar cursores sobre el canal dZ/dt. Transformar coordenada y con la misma normalización aproximada.
        dfc = canales["df_curva"]
        yd = dfc["amplitud_relativa"].to_numpy(float)
        yd_norm = norm(yd, 0.80) + 2.2
        tt = dfc["tiempo_ms"].to_numpy(float)
        for lab, key in [("QRS", "idx_q"), ("B", "idx_b"), ("C", "idx_c"), ("X", "idx_x")]:
            if key in puntos:
                idx = int(puntos[key])
                idx = max(0, min(idx, len(tt)-1))
                ax.scatter(tt[idx], yd_norm[idx], s=70, zorder=5)
                ax.annotate(lab, (tt[idx], yd_norm[idx]), xytext=(5, 8), textcoords="offset points", fontsize=11, fontweight="bold")
                ax.axvline(tt[idx], linestyle="--", linewidth=1.1, alpha=0.7)

    ax.set_xlim(0, rr)
    ax.set_yticks([])
    ax.set_xlabel("Tiempo (ms) - grillado vertical cada 20 ms")
    ax.set_title("Digitalizacion automatica del sector superior derecho: dZ/dt + ECG + Fonocardiograma")
...
def actualizar_aprendizaje_cursores(pauto: Dict[str, Any], pfinal: Dict[str, Any], validado: bool) -> Dict[str, Any]:
    """Registra aprendizaje semiautomatico: número de validaciones y error medio de cursores."""
    estado = st.session_state.get("aprendizaje_cursores", {"n": 0, "error_ms_acum": 0.0})
    if validado:
        diffs = []
        for k in ["q_ms", "b_ms", "c_ms", "x_ms"]:
            if k in pauto and k in pfinal:
                diffs.append(abs(float(pfinal[k]) - float(pauto[k])))
...
def mostrar_barra_aprendizaje() -> None:
    estado = st.session_state.get("aprendizaje_cursores", {"n": 0, "error_ms_prom": None})
    n = int(estado.get("n", 0))
    progreso = min(1.0, n / 30.0)
    st.progress(progreso, text=f"Aprendizaje semiautomatico de cursores: {n}/30 validaciones")
    if estado.get("error_ms_prom") is not None:
        st.caption(f"Correccion media historica de cursores: {estado['error_ms_prom']:.1f} ms. Esta barra documenta aprendizaje operativo; no modifica automaticamente los cursores sin confirmacion medica.")
...
if modo_morfo.startswith("PDF"):
    st.info(
        "El recorte automatico toma el panel superior derecho del informe: banda superior dZ/dt, banda media ECG y banda inferior fonocardiograma. "
    ...
    if pdf_bytes_auto is not None:
        ...
        if "canales_zlogic_pdf" in st.session_state:
            ...
            fig_canales_pre = graficar_canales_zlogic(canales, st.session_state.get("puntos_qbcx_final"))
...
if "df_curva_icg" in st.session_state and "puntos_qbcx_auto" in st.session_state:
    ...
    if "canales_zlogic_pdf" in st.session_state:
        fig_canales = graficar_canales_zlogic(st.session_state["canales_zlogic_pdf"], pfinal)
