...
def filtrar_metricas_pdf_morfologicas(df_der: Optional[pd.DataFrame]) -> pd.DataFrame:
    """Deja solo métricas calculadas/derivadas, evitando tabla de valores importados del equipo."""
    if df_der is None or df_der.empty:
        return pd.DataFrame()
    df2 = df_der.copy()
    var = df2.get("Variable", pd.Series(dtype=str)).astype(str).str.lower()
    estado = df2.get("Estado", pd.Series(dtype=str)).astype(str).str.lower()
    excluir = var.str.contains("importado", na=False) | estado.str.contains("importad", na=False)
    return df2.loc[~excluir].reset_index(drop=True)


def diagnostico_hemodinamico_desde_morfologia(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> str:
    """Diagnóstico resumido usando métricas calculadas a partir de cursores validados."""
    if not bool((puntos or {}).get("validado_operador", False)):
        return "Los cursores QRS-B-C-X no fueron confirmados por el operador. No se emite diagnóstico hemodinámico definitivo desde la curva."
    ci = _metric_contains(df_der, "CI")
    irv = _metric_contains(df_der, "SVRI") or _metric_contains(df_der, "IRV")
    fe = _metric_value(df_der, "FE Capan")
    vac = _metric_value(df_der, "Acoplamiento VA Ea/Ees")
    pep = _metric_value(df_der, "PEP morfologico") or limpiar_numero((puntos or {}).get("pep_ms"))
    lvet = _metric_value(df_der, "LVET morfologico") or limpiar_numero((puntos or {}).get("lvet_ms"))
    strv = _metric_value(df_der, "STR/CTS morfologico")

    partes = []
    # Patrón hemodinámico principal por IC/IRV cuando están disponibles.
    patron = None
    if ci is not None and irv is not None:
        if ci < 2.5 and irv > 2500:
            patron = "patrón hipodinámico con resistencia vascular periférica elevada"
        elif ci > 4.4 and irv < 1300:
            patron = "patrón hiperdinámico con resistencia vascular periférica reducida"
        elif 2.5 <= ci <= 4.4 and 1300 <= irv <= 2500:
            patron = "patrón normodinámico"
        elif irv > 2500:
            patron = "predominio vasoconstrictor / poscarga periférica aumentada"
        elif ci < 2.5:
            patron = "bajo índice cardíaco relativo"
        elif ci > 4.4:
            patron = "alto índice cardíaco relativo"
    if patron:
        partes.append(f"El análisis morfológico validado sugiere {patron}.")
    else:
        partes.append("El patrón hemodinámico global no puede clasificarse completamente por falta de IC/IRV recalculables confiables.")

    if fe is not None:
        if fe < 0.50:
            partes.append(f"La FE Capan estimada es reducida ({fe*100:.1f}%).")
        elif fe <= 0.70:
            partes.append(f"La FE Capan estimada se encuentra en rango conservado ({fe*100:.1f}%).")
        else:
            partes.append(f"La FE Capan estimada es alta/hiperdinámica ({fe*100:.1f}%).")
    if vac is not None:
        partes.append(f"El acoplamiento ventrículo-arterial Ea/Ees es {vac:.2f}: {interpretar_acoplamiento_vac(vac)}")
    if pep is not None and lvet is not None:
        partes.append(f"Los tiempos validados fueron PEP QRS-B {pep:.0f} ms y LVET B-X {lvet:.0f} ms.")
    if strv is not None:
        partes.append(f"El cociente sistólico PEP/LVET fue {strv:.2f}.")
    return " ".join(partes)


def recomendacion_terapeutica_desde_morfologia(datos: Dict[str, Any], df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> str:
    """Recomendación prudente si hay tratamiento farmacológico informado."""
    medicacion = str(datos.get("medicacion") or "").strip()
    if not medicacion:
        return "No se identificó tratamiento farmacológico en el informe original; no se redacta recomendación de optimización farmacológica."
    if not bool((puntos or {}).get("validado_operador", False)):
        return "Existe tratamiento farmacológico informado, pero los cursores no fueron validados; se recomienda no modificar tratamiento basándose en esta digitalización preliminar."
    ...
    if irv is not None and irv > 2500:
        recomendaciones.append("Por el predominio de resistencia/poscarga elevada, conviene revisar si el esquema antihipertensivo ofrece vasodilatación suficiente y control sostenido de la presión arterial.")
    if ci is not None and ci < 2.5:
        recomendaciones.append("Si el índice cardíaco recalculado es bajo, evitar intensificación que reduzca en exceso frecuencia cardíaca o contractilidad sin reevaluación clínica, ECG y función ventricular.")
    if vac is not None and vac > 1.3:
        recomendaciones.append("El desacoplamiento Ea/Ees sugiere priorizar reducción de carga arterial y reevaluación de acoplamiento luego de optimizar presión arterial.")
    if fe is not None and fe < 0.50:
        recomendaciones.append("La FE Capan reducida obliga a correlacionar con ecocardiograma y clínica antes de ajustar fármacos.")
    if any(x in med_low for x in ["losartan", "enalapril", "valsartan", "telmisartan", "irbesartan"]):
        recomendaciones.append("Ya figura bloqueo del sistema renina-angiotensina; verificar dosis, adherencia, función renal y potasio.")
    if any(x in med_low for x in ["nifedipina", "amlodipina", "lercanidipina"]):
        recomendaciones.append("Ya figura calcioantagonista dihidropiridínico; revisar respuesta tensional, edema y necesidad de ajuste horario/dosis.")
    if any(x in med_low for x in ["doxazosina", "prazosina", "terazosina"]):
        recomendaciones.append("Con alfa-bloqueo informado, controlar hipotensión ortostática y relacionar con síntomas y respuesta de pie si está disponible.")
    recomendaciones.append("La recomendación es orientativa y debe integrarse con PA seriada/MAPA, laboratorio, función renal, potasio, síntomas y ecocardiografía.")
...
def generar_pdf_informe(df: pd.DataFrame, morfo: Optional[Dict[str, Any]] = None) -> bytes:
    """
    PDF enfocado SOLO en el análisis morfológico de la onda de impedancia validada por el operador.
    No genera informe basado en variables hemodinámicas extraídas como resultado final del PDF original.
    """
    if A4 is None:
    ...
    styles.add(ParagraphStyle(name="Advertencia", parent=styles["BodyText"], fontSize=8.4, leading=10.5, textColor=colors.HexColor("#7C2D12")))
    ...
    elems.append(Paragraph("Análisis morfológico validado de onda de impedancia", styles["TituloAzul"]))
    ...
    tabla_datos = [
        ...
        ["Condición", "Curva dZ/dt validada" if validado else "Curva dZ/dt no validada", "Archivo", safe_pdf_text(datos.get("archivo_origen", ""))],
    ]
    ...
    if not validado:
        elems.append(Paragraph("Advertencia: los cursores QRS-B-C-X no fueron confirmados visualmente por el operador. Este PDF no emite diagnóstico hemodinámico definitivo; solo documenta una digitalización preliminar.", styles["Advertencia"]))
        elems.append(Spacer(1, 6))

    png_multi = morfo.get("png_canales")
    if png_multi:
        elems.append(Paragraph("Registro multicanal de apoyo: dZ/dt + ECG + marcas fonocardiográficas", styles["Sub"]))
        elems.append(Paragraph("El fonocardiograma se utiliza como guía temporal mediante barras de S1/S2; no se informa como curva cuantitativa.", styles["NormalSmall"]))
        elems.append(RLImage(io.BytesIO(png_multi), width=17.5*cm, height=7.2*cm))
        elems.append(Spacer(1, 8))

    png = morfo.get("png")
    if png:
        elems.append(Paragraph("Onda dZ/dt validada con cursores QRS-B-C-X", styles["Sub"]))
        elems.append(RLImage(io.BytesIO(png), width=17.5*cm, height=7.2*cm))
        elems.append(Spacer(1, 8))

    elems.append(Paragraph("Cursores validados y tiempos sistólicos", styles["Sub"]))
    ...
    elems.append(Paragraph("Métricas hemodinámicas derivadas de la curva validada", styles["Sub"]))
    if not df_der_pdf.empty:
        ...
        tmet = Table(data_m, colWidths=[4.2*cm, 2.6*cm, 3.1*cm, 7.0*cm], repeatRows=1)
        ...
        elems.append(Paragraph("No se calcularon métricas derivadas suficientes desde la curva.", styles["NormalSmall"]))
    elems.append(Spacer(1, 8))

    elems.append(Paragraph("Diagnóstico hemodinámico integrado desde la curva validada", styles["Sub"]))
    diag = diagnostico_hemodinamico_desde_morfologia(df_der_pdf, puntos)
    elems.append(Paragraph(safe_pdf_text(diag), styles["NormalSmall"]))
    elems.append(Spacer(1, 6))

    elems.append(Paragraph("Recomendación terapéutica orientativa", styles["Sub"]))
    reco = recomendacion_terapeutica_desde_morfologia(datos, df_der_pdf, puntos)
    elems.append(Paragraph(safe_pdf_text(reco), styles["NormalSmall"]))
    elems.append(Spacer(1, 6))

    vars_ct_pdf = calcular_contractilidad_morfologica(morfo.get("df_curva"), puntos, z0=limpiar_numero(datos.get("z0")))
    interp_ct_pdf = interpretar_contractilidad_morfologica(vars_ct_pdf)
    if vars_ct_pdf.get("dzdt_max_rel") is not None:
        elems.append(Paragraph("Contractilidad morfológica relativa", styles["Sub"]))
        filas_ct = [["Variable", "Valor"]]
        for etiqueta, clave in [
            ("dZ/dt máximo relativo", "dzdt_max_rel"),
            ("Índice de Heather relativo", "hi_rel"),
            ("Índice de velocidad relativo", "vi_rel"),
            ("Índice de aceleración relativo", "aci_rel"),
            ("Pendiente ascendente B-C", "pendiente_ascendente"),
            ("Pendiente descendente C-X", "pendiente_descendente"),
            ("Área sistólica relativa B-X", "area_sistolica_rel"),
            ("Simetría sistólica", "simetria_sistolica"),
        ]:
    ...
    elems.append(Paragraph("Nota metodológica: este PDF no reproduce el informe hemodinámico original ni utiliza las variables extraídas del PDF como resultado principal. Los cálculos se informan como derivados del análisis morfológico de la onda dZ/dt con cursores QRS-B-C-X validados por el operador; las variables que requieren presión arterial, antropometría o volumen sistólico usan esos datos solo como entradas auxiliares para las ecuaciones.", styles["NormalSmall"]))
