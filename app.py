...
def tabla_dominios_hemodinamicos(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {"Dominio": "Flujo", "Variables": "IC, IDS, IRV", "Valores": f"IC {fmt(d['ci'],2)} L/min/m²; IDS {fmt(d['ids'],1)} mL/lat/m²; IRV {fmt(d['irv'],0)}", "Interpretación": f"{d['flujo_ic']}; {d['flujo_ids']}; {d['flujo_irv']}"},
        {"Dominio": "Función cardíaca", "Variables": "FE Capan", "Valores": f"FE Capan {fmt(fe_pct,1)} %", "Interpretación": d["funcion"]},
        {"Dominio": "Contractilidad", "Variables": "IV, IH, IAC, ITC, relación de tiempos sistólicos", "Valores": f"IV {fmt(d['iv'],4)}; IH {fmt(d['ih'],5)}; IAC {fmt(d['iac'],4)}; ITC {fmt(d['itc'],2)}; PEP/LVET {fmt(d['rts'],3)}", "Interpretación": "Perfil contráctil morfológico derivado de la onda validada"},
        {"Dominio": "Rendimiento cardiovascular", "Variables": "Acoplamiento VA Ea/Ees", "Valores": f"AC {fmt(d['ac'],3)}", "Interpretación": d["rendimiento"]},
        {"Dominio": "Volemia", "Variables": "CFT/TFC", "Valores": f"CFT {fmt(d['cft'],1)} 1/kOhm", "Interpretación": d["volemia"]},
    ])
...
def construir_grafico_terapeutico_png(datos: Dict[str, Any], df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> Optional[bytes]:
    """Crea un gráfico terapéutico tipo algoritmo, señalando la rama activa.

    Se diseña en formato ancho para PDF, con texto envuelto y cajas amplias
    para evitar superposición. No mezcla valores métricos en las conclusiones:
    solo muestra patrones y ramas de optimización.
    """
    try:
        ramas = ramas_terapeuticas_desde_morfologia(df_der, puntos)
        d = construir_dominios_hemodinamicos(df_der, puntos)
        medicacion = str(datos.get("medicacion") or "No informada").strip()
        ...
        ax.text(0.035, 0.95, "Algoritmo de optimización terapéutica guiado por ICG", fontsize=21,
                fontweight="bold", color=navy, va="center")
        ax.text(0.035, 0.905,
                "La rama resaltada se deriva del análisis morfológico dZ/dt con cursores QRS-B-C-X y dominios hemodinámicos.",
                fontsize=9.5, color="#475569", va="center")

        # Encabezados
        headers = [
            (0.05, 0.82, 0.20, 0.08, "Evaluación"),
            (0.31, 0.82, 0.18, 0.08, "Perfil\nhemodinámico"),
            (0.55, 0.82, 0.18, 0.08, "Implicancia\ndiagnóstica"),
            (0.78, 0.82, 0.18, 0.08, "Rama de\ntratamiento"),
        ]
        for x, y, w, h, txt in headers:
            ax.add_patch(plt.Rectangle((x, y), w, h, facecolor=blue if x < 0.75 else "#FFD28A",
                                       edgecolor=navy, linewidth=1.1))
            ax.text(x+w/2, y+h/2, txt, ha="center", va="center", fontsize=10.5,
                    color=navy, fontweight="bold", linespacing=1.15)

        # Caja evaluación
        ax.add_patch(plt.Rectangle((0.05, 0.55), 0.20, 0.13, facecolor="#F2FAFC", edgecolor=inactive_edge, linewidth=1.0))
        ax.text(0.15, 0.615, "Historia clínica\nPA, laboratorio\n+ BioZ / CGI", ha="center", va="center", fontsize=10.2, color=navy)

        rows = [
            {
                "key": "hiperdinamico", "y": 0.66, "perfil": "Hiperdinámico", "diag": "Flujo alto", 
                "tx": "Agregar o aumentar beta-bloqueante o calcioantagonista no dihidropiridínico si el contexto clínico lo permite.",
                "color": teal,
            },
            {
                "key": "hipodinamico", "y": 0.50, "perfil": "Hipodinámico", "diag": "Bajo flujo", 
                "tx": "Reducir beta-bloqueo si está presente y no existe indicación obligatoria; reevaluar función ventricular y volumen efectivo.",
                "color": burg,
            },
            {
                "key": "vasoconstrictor", "y": 0.34, "perfil": "Vasoconstrictor", "diag": "Poscarga elevada", 
                "tx": "Optimizar IECA/ARA-II, calcioantagonista dihidropiridínico o vasodilatador directo según tolerancia, función renal y potasio.",
                "color": teal,
            },
            {
                "key": "retencion_fluido", "y": 0.18, "perfil": "Retención de fluidos", "diag": "Hipervolemia", 
                "tx": "Agregar o aumentar diurético si la evaluación clínica confirma retención de fluidos.",
                "color": "#0F766E",
            },
        ]

        # Si hay hipovolemia, reemplaza visualmente la rama de fluidos para que la orientación sea segura.
        if ramas.get("hipovolemia") and not ramas.get("retencion_fluido"):
            rows[-1]["perfil"] = "Hipovolemia"
            rows[-1]["diag"] = "Volumen bajo"
            rows[-1]["tx"] = "Evitar intensificación diurética; revisar volumen efectivo, síntomas ortostáticos y función renal."
            rows[-1]["key"] = "hipovolemia"
            rows[-1]["color"] = "#0369A1"

        def box(x, y, w, h, text, active_flag=False, color=navy, fontsize=9.4, bold=False):
            fc = active if active_flag else "#FFFFFF"
            ec = active_edge if active_flag else inactive_edge
            lw = 2.3 if active_flag else 0.9
            ax.add_patch(plt.Rectangle((x, y), w, h, facecolor=fc, edgecolor=ec, linewidth=lw))
            wrapped = "\n".join(textwrap.wrap(str(text), width=max(12, int(w * 95))))
            ax.text(x+0.012, y+h/2, wrapped, ha="left", va="center", fontsize=fontsize,
                    color=color if active_flag else "#111827", fontweight="bold" if bold else "normal",
                    linespacing=1.15)

        for row in rows:
            y = row["y"]
            is_active = bool(ramas.get(row["key"], False))
            # flechas desde evaluación y entre columnas
            ax.annotate("", xy=(0.31, y+0.045), xytext=(0.25, 0.615), arrowprops=dict(arrowstyle="->", color="#B6C3CC", lw=1.1))
            ax.annotate("", xy=(0.55, y+0.045), xytext=(0.49, y+0.045), arrowprops=dict(arrowstyle="->", color="#B6C3CC", lw=1.1))
            ax.annotate("", xy=(0.78, y+0.045), xytext=(0.73, y+0.045), arrowprops=dict(arrowstyle="->", color="#B6C3CC", lw=1.1))
            box(0.31, y, 0.18, 0.09, row["perfil"], is_active, row["color"], fontsize=10.0, bold=True)
            box(0.55, y, 0.18, 0.09, row["diag"], is_active, navy, fontsize=9.6)
            box(0.78, y-0.015, 0.19, 0.12, row["tx"], is_active, navy, fontsize=8.1)
            if is_active:
                ax.text(0.755, y+0.045, "➜", fontsize=20, color=active_edge, ha="center", va="center", fontweight="bold")

        med_txt = "Tratamiento informado: " + medicacion
        ax.text(0.05, 0.07, "\n".join(textwrap.wrap(med_txt, width=145)), fontsize=8.4, color="#475569",
                bbox=dict(boxstyle="round,pad=0.35", facecolor="#F8FAFC", edgecolor="#CBD5E1"))
        ax.text(0.05, 0.035,
                "Orientación clínica: correlacionar con PA clínica/MAPA, función renal, potasio, síntomas, ECG y ecocardiograma antes de modificar tratamiento.",
...
def generar_pdf_informe(df: pd.DataFrame, morfo: Optional[Dict[str, Any]] = None) -> bytes:
    ...
    tdom = Table(data_dom, colWidths=[3.0*cm, 4.2*cm, 4.8*cm, 5.0*cm], repeatRows=1)
    tdom.setStyle(TableStyle([("BACKGROUND", (0,0), (-1,0), colors.HexColor("#0B4F7A")), ("TEXTCOLOR", (0,0), (-1,0), colors.white), ("GRID", (0,0), (-1,-1), .3, colors.HexColor("#D7E3EE")), ("VALIGN", (0,0), (-1,-1), "TOP"), ("FONTSIZE", (0,0), (-1,-1), 6.6)]))
    ...
    if not df_der_pdf.empty:
        ...
        for _, rr_m in df_der_pdf.iterrows():
            data_m.append([safe_pdf_text(rr_m.get("Variable", "")), safe_pdf_text(rr_m.get("Valor recalculado", "")), safe_pdf_text(rr_m.get("Unidad", "")), safe_pdf_text(rr_m.get("Estado", ""))])
        tmet = Table(data_m, colWidths=[4.2*cm, 2.4*cm, 3.0*cm, 7.2*cm], repeatRows=1)
        tmet.setStyle(TableStyle([("BACKGROUND", (0,0), (-1,0), colors.HexColor("#0B4F7A")), ("TEXTCOLOR", (0,0), (-1,0), colors.white), ("GRID", (0,0), (-1,-1), .3, colors.HexColor("#D7E3EE")), ("VALIGN", (0,0), (-1,-1), "TOP"), ("FONTSIZE", (0,0), (-1,-1), 6.6)]))
        elems.append(tmet)
    else:
    ...
    elems.append(Paragraph("Sugerencia terapéutica orientativa por rama del algoritmo", styles["Sub"]))
    if grafico_tx:
        elems.append(RLImage(io.BytesIO(grafico_tx), width=17.5*cm, height=9.8*cm))
    ...
    elems.append(Spacer(1, 6))
    elems.append(Paragraph("Nota metodológica: este PDF deja exclusivamente las variables solicitadas para análisis por dominios de la onda dZ/dt con cursores QRS-B-C-X. No reproduce ni informa la tabla completa de variables extraídas del PDF original.", styles["NormalSmall"]))
    doc.build(elems); buffer.seek(0); return buffer.getvalue()
