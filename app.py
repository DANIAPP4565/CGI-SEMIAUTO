...
def _digitalizar_banda_trazo(
    ...
    Corrección importante para Z-Logic:
    - El canal inferior es el fonocardiograma. En muchos informes aparece sobre una franja
      amarilla/resaltada. Si se usan todos los pixeles coloreados, la app puede confundir
      el relleno amarillo o volver a seguir el ECG.
    - Por eso se prioriza el trazo azul/oscuro y se excluye el relleno amarillo antes de
      seguir la curva por continuidad.
    """
    h, w = roi_rgb.shape[:2]
    ...
    # Curvas Z-Logic: suelen ser azul oscuro; el fonocardiograma inferior puede estar
    # dibujado sobre una banda amarilla. Excluir el amarillo evita que el relleno sea
    # interpretado como curva.
    r = bf[:, :, 0]
    g = bf[:, :, 1]
    b = bf[:, :, 2]

    mask_amarillo = (r > 150) & (g > 130) & (b < 130) & (sat > 45)
    mask_azul = (b > r + 12) & (b > g + 4) & (gris < 245)
    mask_oscura = (gris < umbral_oscuridad)
    mask_color_no_amarillo = (sat > 35) & (gris < 230)
    if excluir_relleno_amarillo:
        mask_color_no_amarillo = mask_color_no_amarillo & (~mask_amarillo)
        mask_oscura = mask_oscura & (~mask_amarillo)

    # Prioridad: trazo azul/oscuro de la curva. En el fono inferior, NO usar la
    # máscara de color general porque captura la franja amarilla.
    if preferir_trazo_azul and mask_azul.sum() >= 15:
        mask = mask_azul
    elif mask_oscura.sum() >= 15:
        mask = mask_oscura
    elif mask_color_no_amarillo.sum() >= 15:
        mask = mask_color_no_amarillo
    else:
        mask = (gris < min(190, umbral_oscuridad + 35))
        if excluir_relleno_amarillo:
            mask = mask & (~mask_amarillo)

    mask = _limpiar_mascara_grafica(mask)
    if mask.sum() < 15:
        # Rescate final: solo trazos azules u oscuros, nunca relleno amarillo.
        mask = _limpiar_mascara_grafica((mask_azul | mask_oscura) & (~mask_amarillo))
...
def digitalizar_pdf_zlogic_sector_superior_derecho(
    ...
    df_fono = _digitalizar_banda_trazo(
        arr, 0.735, 0.885, rr, umbral_oscuridad=150, suavizado=max(1, min(3, suavizado)),
        invertir=False, max_salto_px=max(3, int(h*0.035)),
        preferir_trazo_azul=True, excluir_relleno_amarillo=True, nombre_canal="fono_inferior"
    )
...
def graficar_canales_zlogic(
    ...
    amplificar_fono: float = 2.60,
    amplificar_ecg: float = 1.15,
    ...
    y_ecg = norm(df_ecg["amplitud"].to_numpy(float), 0.55 * float(amplificar_ecg)) + base_ecg
    y_fono = norm(df_fono["amplitud"].to_numpy(float), 0.70 * float(amplificar_fono)) + base_fono
...
if modo_morfo.startswith("PDF"):
    ...
    with colamp1:
        amplificar_ecg = st.slider("Amplificación visual ECG verde", 0.7, 2.5, 1.15, 0.05)
    with colamp2:
        amplificar_fono = st.slider("Amplificación visual fonocardiograma naranja inferior", 1.0, 5.0, 2.60, 0.05)
...
if "df_curva_icg" in st.session_state and "puntos_qbcx_auto" in st.session_state:
    ...
    if "canales_zlogic_pdf" in st.session_state:
        fig_canales = graficar_canales_zlogic(st.session_state["canales_zlogic_pdf"], pfinal, amplificar_fono=float(st.session_state.get("amplificar_fono_visual", 2.60)), amplificar_ecg=float(st.session_state.get("amplificar_ecg_visual", 1.15)), expandir_vertical=True)
