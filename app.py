...
def diagnostico_hemodinamico_desde_morfologia(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> str:
    ...
    fe_pct = None if d["fe"] is None else (d["fe"] * 100 if d["fe"] <= 1.5 else d["fe"])
    return " ".join([
        f"Dominio de flujo: {d['flujo_ic']} por IC {fmt(d['ci'],2)} L/min/m², IDS {fmt(d['ids'],1)} mL/lat/m² e IRV {fmt(d['irv'],0)} ({d['flujo_irv']}).",
        f"Función cardíaca: FE Capan {fmt(fe_pct,1)}%, compatible con {d['funcion'].lower()}.",
        f"Contractilidad: IV {fmt(d['iv'],4)}, IH {fmt(d['ih'],5)}, IAC {fmt(d['iac'],4)}, ITC {fmt(d['itc'],2)} y relación PEP/LVET {fmt(d['rts'],3)}.",
        f"Rendimiento cardiovascular: acoplamiento VA {fmt(d['ac'],3)}, clasificado como {d['rendimiento'].lower()}.",
        f"Volemia: CFT/TFC {fmt(d['cft'],1)}, compatible con {d['volemia'].lower()}.",
    ])
...
def recomendacion_terapeutica_desde_morfologia(datos: Dict[str, Any], df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> str:
    ...
    if not medicacion:
        return "No se identificó tratamiento farmacológico informado; no se redacta recomendación de optimización farmacológica."
    if not bool((puntos or {}).get("validado_operador", False)):
        return "Existe tratamiento farmacológico informado, pero los cursores QRS-B-C-X no fueron validados; no se recomienda optimizar tratamiento basándose en esta digitalización preliminar."
    d = construir_dominios_hemodinamicos(df_der, puntos)
...
def construir_matriz_perfiles_png(df_der: Optional[pd.DataFrame], puntos: Dict[str, Any]) -> Optional[bytes]:
    ...
    try:
        ...
        ax.text(0.055, 0.043,
                "El resaltado del cuadrante requiere cursores QRS-B-C-X confirmados por el operador.",
                fontsize=9.3, color="#7C2D12")
...
def generar_pdf_informe(df: pd.DataFrame, morfo: Optional[Dict[str, Any]] = None) -> bytes:
    ...
    elems.append(Paragraph("Análisis morfológico validado de onda de impedancia dZ/dt", styles["TituloAzul"]))
    ...
    tabla_datos = [["Paciente", safe_pdf_text(paciente), "Estado", "Cursores validados" if validado else "Cursores no validados"], ["Base analítica", "Onda dZ/dt con QRS-B-C-X", "Archivo", safe_pdf_text(datos.get("archivo_origen", ""))]]
    ...
    if not validado:
        elems.append(Paragraph("Advertencia: los cursores QRS-B-C-X no fueron confirmados visualmente por el operador. Este PDF no emite diagnóstico hemodinámico definitivo.", styles["Advertencia"])); elems.append(Spacer(1, 6))
    ...
    elems.append(Spacer(1, 8))
    elems.append(Paragraph("Conclusión integrada", styles["Sub"]))
    elems.append(Paragraph(safe_pdf_text(diagnostico_hemodinamico_desde_morfologia(df_der_pdf, puntos)), styles["NormalSmall"])); elems.append(Spacer(1, 6))
    elems.append(Paragraph("Sugerencia terapéutica orientativa", styles["Sub"]))
    elems.append(Paragraph(safe_pdf_text(recomendacion_terapeutica_desde_morfologia(datos, df_der_pdf, puntos)), styles["NormalSmall"])); elems.append(Spacer(1, 6))
    elems.append(Paragraph("Nota metodológica: este PDF deja exclusivamente las variables solicitadas para análisis por dominios de la onda dZ/dt validada. No reproduce ni informa la tabla completa de variables extraídas del PDF original.", styles["NormalSmall"]))
    doc.build(elems); buffer.seek(0); return buffer.getvalue()
